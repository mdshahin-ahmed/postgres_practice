...
-- question 1: retrieve all employees whose salary is greater than the highest salary of the HR department 
...
-- SCALAR sub query -> when return single value
...
-- if we  want to use sub query with select -> sub query must return single value
SELECT *, (SELECT sum(salary) from employees) FROM employees

-- if we  want to use sub query with From
