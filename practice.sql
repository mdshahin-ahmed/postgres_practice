...
-- Question 1: Inner join to retrieve employee and department information
...
-- Question 2: Show department name with average salary
SELECT department_name,round(avg(salary)) as avg_salary from employees
JOIN departments using(department_id)
GROUP BY department_name

-- Question 3: Count employees in each department
SELECT department_name,count(*) from employees
JOIN departments using(department_id)
GROUP BY department_name

-- Question 4: Find the department name with the highest average salary
SELECT department_name,round(avg(salary)) as avg_salary from employees
JOIN departments using(department_id)
GROUP BY department_name
...
-- Question 5: Count employees hired each YEAR
...
-- Question 6: Find customers who have placed more than 2 orders and calculate the total amount spent by each of these customers.
SELECT customer_id,count(order_id),sum(total_amount) as total_spent FROM orders
GROUP BY customer_id
HAVING count(order_id) > 2 

-- Question 7: Find the total amount of orders placed each month in the year 2022
